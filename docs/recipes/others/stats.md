	...
	return rx.card(
		rx.vstack(
			...
			rx.hstack(
				...
				rx.text(f"{change} from last month", size="2", color=rx.color("gray", 10),  white_space="nowrap"),
				align="center",
				width="100%",
			),
			spacing="3",
		...
		rx.hstack(
			rx.vstack(
				...
				rx.hstack(
					...
					rx.text(f"from {prev_value:,}", size="3", color=rx.color(
						"gray", 10),  white_space="nowrap"),
					spacing="2",
					align_items="end",
				),
				align_items="start",
			...
			rx.badge(
				...
				rx.text(f"{percentage_change}%", size="2",
						color=rx.color(arrow_color, 9), weight="medium"),
