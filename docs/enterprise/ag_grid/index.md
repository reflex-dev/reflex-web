...
column_defs = [
    dict(field="country", headerName="Country", filter=True),
    dict(field="pop", headerName="Population"),
    dict(field="lifeExp", headerName="Life Expectancy", filter=True),
]
...
column_defs = [
    dict(field="Task", filter=True),
    dict(field="Start", filter=rxe.ag_grid.filters.date),
    dict(field="Duration", filter=rxe.ag_grid.filters.number),
    dict(field="Resource", filter=rxe.ag_grid.filters.text),
]
...
column_defs = [
    dict(field="country", sortable=False),
    dict(field="pop", headerName="Population"),
    dict(field="lifeExp", headerName="Life Expectancy"),
]
...
column_defs = [
    dict(field="country", checkboxSelection=True),
    dict(field="pop", headerName="Population"),
    dict(field="continent"),
]
...
def ag_grid_simple_row_selection():
    return rxe.ag_grid(
        ...
        row_data=df.to_dict("records"),
        column_defs=column_defs,
        row_selection=dict(mode="multiple"),
        width="100%",
        height="40vh",
    )
```
...
column_defs = [
    dict(field="country"),
    dict(field="pop", headerName="Population", editable=True, cellEditor=rxe.ag_grid.editors.number),
    dict(field="continent", editable=True, cellEditor=rxe.ag_grid.editors.select, cellEditorParams=dict(values=['Asia', 'Europe', 'Africa', 'Americas', 'Oceania'])),
]
...
column_defs = [
    dict(field="country"),
    dict(field="population", headerName="Population", editable=True, cellEditor=rxe.ag_grid.editors.number),
    dict(field="continent", editable=True, cellEditor=rxe.ag_grid.editors.select, cellEditorParams=dict(values=['Asia', 'Europe', 'Africa', 'Americas', 'Oceania'])),
]
...
def api_info_callout():
    return rx.callout(
        rx.text("Another way to use the AG Grid API"),
        rx.text("It is also possible to use the AG Grid API directly with the event trigger (on_click) of the component. This removes the need to create a variable my_api. This is shown in the example below. It is necessary to use the id of the ag_grid component that is to be referenced."),
        icon="info",
        color_scheme="blue",
    )
```
