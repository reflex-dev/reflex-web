...
def docs_item(
    ...
    return rx.el.div(
        rx.el.div(
            ui.icon(icon, class_name="size-6", stroke_width=1.5),
            rx.el.span(
                title,
            ...
            ui.icon(
                "ArrowRight01Icon",
                class_name="size-4 ml-auto group-hover:text-primary-10",
            ),
            class_name="flex row items-center gap-3 h-8",
        ),
        rx.el.p(description, class_name="text-secondary-11 text-sm font-[475]"),
        rx.el.a(to=href, class_name="absolute inset-0"),
        class_name="flex flex-col gap-2 py-8 pr-8 relative group lg:max-w-[21rem] w-full max-lg:text-start",
    )
