...
for doc in flexdown_docs:
    if not doc.startswith("docs/getting-started") and not doc.startswith("docs/library/typography") and not doc.startswith("docs/library/feedback") and not doc.startswith("docs/library/overlay"):
        continue
