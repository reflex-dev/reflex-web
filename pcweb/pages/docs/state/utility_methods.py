...
def utility_methods():
    return rx.box(
        ...
        rx.unordered_list(
            rx.list_item(
                rx.text(rx.code("get_client_ip()"), ": Returns the IP of current user."),
            ),
            rx.list_item(
                rx.text(rx.code("get_current_page()"), ": Returns the URL the current page."),
            ),
            rx.list_item(
                rx.text(rx.code("get_cookies()"), ": Returns the current user's browser cookies."),
            ),
            padding_bottom="0.7em"
        ),
        doccode(code1),
        doctext("Check out the ",
                doclink("State class definition",
