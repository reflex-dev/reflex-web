...
def video_demo() -> rx.Component:
    return rx.el.section(
        ...
        rx.box(
            rx.el.span(
                "Get one month free",
                class_name="rounded-full border border-slate-6 text-slate-12 text-xs px-2 py-0.5 w-fit font-medium -mb-2",
            ),
            rx.el.span(
                rx.el.span(
                    "Reflex Build ",
                    class_name="inline text-slate-12 font-semibold text-3xl",
                ),
                rx.el.span(
                    "The first AI agent to build internal Python enterprise apps.",
                    class_name="inline text-slate-9 font-medium text-3xl",
                ),
                class_name="flex flex-col gap-1",
            ),
            rx.link(
                button("Get Early Access", size="xl", class_name="w-fit"),
                href="/pricing",
            ),
        ...
        dialog(
            trigger=rx.box(
                ...
                rx.el.span(
                    class_name="inset-0 size-full absolute z-[1] bg-[#00000028] backdrop-blur-[1.5px] rounded-lg",
                ),
        ...
        class_name="border-slate-3 bg-slate-2/80 lg:border lg:rounded-[1.125rem] w-[calc(100%+2rem)] -mx-4 h-full overflow-hidden shadow-large lg:w-full lg:-mx-0 max-w-[69.25rem] backdrop-blur-sm p-8 grid grid-cols-1 lg:grid-cols-2 gap-12 relative isolate bg-slate-2",
    )
